...
def main()->int:
 p=argparse.ArgumentParser();p.add_argument('--report',default='evals/reports/phase80-release-candidate.json');a=p.parse_args()
 r=evaluate_release_candidate(ROOT)
 payload={'phase':80,'release':r.release,'status':r.status,'critical_failures':r.critical_failures,'ci_browser_required':r.ci_browser_required,'gates':[g.__dict__ for g in r.gates]}
