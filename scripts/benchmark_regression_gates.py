...
def manifest(run_id: str, score: float, model: str, prompt: str, retriever: str) -> RunManifest:
    return RunManifest(
        run_id=run_id, dataset_id="phase60-ci", dataset_version="1.0.0", dataset_sha256="6"*64,
        evaluator_names=["quality","safety"], runner_name="phase60-benchmark", total_cases=100, passed_cases=100,
        ...
        reproducibility=ReproducibilityMetadata(python_version="3.12",platform="ci",git_commit=None,git_dirty=None,seed=60,dependency_fingerprint="phase60",environment="ci"),
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as temp:
        ...
        baseline=manifest("phase60-baseline",.955,"gpt-standard","1","dense")
        candidate=manifest("phase60-candidate",.962,"gpt-standard","2","fused")
        ...
        if not decision.releasable or not decision.baseline_promotable:
            raise SystemExit("Phase 60 regression gate failed")
        store.promote_baseline(dataset_id=candidate.dataset_id,environment="ci",run_id=candidate.run_id,promoted_by="phase60-ci",reason="quality gate passed")
        promoted=store.get_baseline(dataset_id=candidate.dataset_id,environment="ci")
        payload={"phase":60,"gate":decision.model_dump(mode="json"),"baseline_promoted":promoted.run_id if promoted else None,"quality_budgets":"blocking_and_warning","statistical_detection":"paired_95pct_ci","flaky_handling":"variance_quarantine","change_attribution":decision.attribution.changed_dimensions}
