# Phase 17 — Video Evidence Pipeline

Phase 17 turns an admitted video object into traceable multimodal incident evidence without placing large media payloads on Kafka.

## Production flow

`S3/MinIO video object -> Kafka object-reference command -> video-evidence-worker -> FFprobe validation/duration -> FFmpeg WAV extraction -> Phase 16 transcription -> FFmpeg scene + interval keyframes -> CPU-safe frame observations -> transcript/frame alignment -> PostgreSQL/RLS -> progress/result events`

## Key decisions

- Kafka transports object references only; video bytes never ride the event bus.
- Worker-side validation re-checks the MP4 signature, byte limit, SHA-256, and FFprobe duration even though Phase 4 already admitted the object.
...
- Phase 16 transcript evidence remains authoritative for spoken statements.
...
- `transcript_statement` — redacted timestamped Phase 16 speech evidence.
...
Phase 17 does not perform semantic video RCA. Later agents may reason over this evidence, but they must preserve the direct-observation/inference distinction and stable evidence identifiers.
