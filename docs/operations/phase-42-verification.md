# Phase 42 Verification and Recovery Runbook
...
## Cancellation
Cancellation writes a durable Phase 42 cancellation event and moves the graph run to CANCELLED through the operations service. Recovery scanners must not resurrect it.

## Replay
Replay is observational. It reconstructs the ordered Phase 42 durability-event stream and returns a canonical hash. It never reissues tool calls or external writes.
