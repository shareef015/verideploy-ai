...
- Modify: any doc/config that names these scripts (caught by the sweep: `docs/architecture/phase-66-*.md`, `docs/phase60-*.md`, `docs/architecture/phase-52-*.md`, `docs/phase-50-handoff.md`, `config/operations/phase79-readiness.json`, `config/monorepo/policy.json`, `generated/clients/python/verideploy_contracts.py` header comment)
...
**Files:**
- Modify: 15 files under `config/` (renamed) — `config/architecture/phase82-production-topology.json` is the highest-fanout one (8 referencing files per the inventory)
...
- [ ] **Step 2: Verify the highest-fanout file specifically (`phase82-production-topology.json`)**

```bash
cd "c:\Users\Administrator\Desktop\IT\OPS\verideploy-ai"
grep -rn "phase82-production-topology\|phase82_production_topology" . --include="*.py" --include="*.ts" --include="*.md" --include="*.json" 2>/dev/null | grep -v node_modules
```
