...
The v1 policy uses transparent factors: change blast radius, large change set, failed workflows, database migration/rollback readiness, recent incidents, coverage regression, critical security findings, and deployment-window risk. Critical security findings, repeated CI failures, and unverified migration rollback paths impose score floors. The LLM is not authoritative for Phase 2 scoring.

## Security boundary

The Python endpoint requires the trusted gateway service identity header and tenant-scoped reads. This is a Phase 2 service-boundary mechanism, not the final production service-auth design; full workload identity is scheduled for the security phase.
