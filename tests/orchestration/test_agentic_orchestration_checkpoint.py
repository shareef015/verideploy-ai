...
from verideploy.orchestration.checkpoint import Phase77Checkpoint, run_phase77_checkpoint

ROOT = Path(__file__).resolve().parents[2]


def test_phase77_all_deterministic_scenarios_complete_and_recover():
...
def test_phase77_retry_is_bounded_and_trace_linked():
    report = run_phase77_checkpoint(ROOT)
...
def test_phase77_fanout_fanin_and_critic_loop_are_complete():
    report = run_phase77_checkpoint(ROOT)
...
def test_phase77_consequential_path_stops_at_human_approval():
    report = run_phase77_checkpoint(ROOT)
...
def test_phase77_path_metrics_are_perfect_for_protected_scenarios():
    report = run_phase77_checkpoint(ROOT)
