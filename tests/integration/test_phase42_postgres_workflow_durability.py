...
def test_phase42_postgres_lease_idempotency_rls_and_append_only_recovery():
    ...
    with engine.begin() as c:
        for t in (tenant,other): c.execute(text('INSERT INTO tenants(tenant_id,slug,display_name) VALUES(:id,:slug,:name) ON CONFLICT DO NOTHING'),{'id':t,'slug':f'p42-{str(t)[:8]}','name':'phase42'})
        c.execute(text("INSERT INTO graph_runs_phase18(run_id,tenant_id,thread_id,graph_name,graph_version,correlation_id,status,last_sequence,created_at,updated_at) VALUES(:r,:t,:th,'p42','1','corr','RUNNING',0,now(),now())"),{'r':run,'t':tenant,'th':str(run)})
    ...
    with db.tenant_session(tenant) as s:
        eid=s.execute(text('SELECT event_id FROM workflow_durability_events_phase42 WHERE run_id=:r ORDER BY sequence LIMIT 1'),{'r':run}).scalar_one()
        with pytest.raises(DBAPIError,match='append-only'):
            s.execute(text("UPDATE workflow_durability_events_phase42 SET event_type='mutated' WHERE event_id=:id"),{'id':eid}); s.commit()
        s.rollback()
        with pytest.raises(DBAPIError,match='terminal'):
            s.execute(text("UPDATE workflow_steps_phase42 SET status='failed' WHERE run_id=:r AND idempotency_key='write:42'"),{'r':run}); s.commit()
        s.rollback()
    db.dispose()
