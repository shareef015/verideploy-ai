...
def test_phase38_postgres_citations_are_tenant_scoped_append_only_and_mapped():
    assert URL
    ...
    try:
        with db.engine.begin() as conn:
            for tenant,slug in ((TENANT,"phase38"),(OTHER,"phase38-other")):
        ...
        with db.session(tenant_id=TENANT) as s:
            s.execute(text("""INSERT INTO retrieval_documents(document_id,tenant_id,source_key,title,service,environment,document_kind,severity,team,occurred_at,required_permission)
                VALUES(:doc,:tenant,'runbook://phase38','Phase38 runbook','checkout','production','runbook','sev1','commerce',now(),'retrieval.read')"""),{"doc":str(doc),"tenant":str(TENANT)})
            ...
            s.execute(text("INSERT INTO self_corrective_rag_runs_phase36(run_id,tenant_id,controller_version,stop_reason,answerable,qualified,result_json) VALUES(:run,:tenant,'1.0.0','sufficient_evidence',true,false,CAST(:payload AS jsonb))"),{"run":str(source_run),"tenant":str(TENANT),"payload":'{"stub":true}'})
            s.execute(text("""INSERT INTO hallucination_protection_runs_phase37(verification_id,tenant_id,self_corrective_run_id,verifier_version,protected,supported_count,uncertain_count,unsupported_count,unsupported_material_rate,prompt_injection_evidence_count,result_json)
        ...
        with pytest.raises(DBAPIError):
            with db.session(tenant_id=TENANT) as s:s.execute(text("UPDATE citations_phase38 SET title='mutated' WHERE citation_id=:id"),{"id":str(cid)})
    finally:db.dispose()
