...
def test_phase41_postgres_concurrency_rls_signed_transition_and_append_only():
    assert URL
    ...
    with engine.begin() as conn:
        for t in (tenant,other):
            conn.execute(text("INSERT INTO tenants(tenant_id,slug,display_name) VALUES(:id,:slug,:name) ON CONFLICT DO NOTHING"),{"id":t,"slug":f"p41-{str(t)[:8]}","name":"phase41"})
        conn.execute(text("""INSERT INTO graph_runs_phase18(run_id,tenant_id,thread_id,graph_name,graph_version,correlation_id,status,last_sequence,created_at,updated_at)
            VALUES(:run,:tenant,:thread,'phase41-live','1','corr','WAITING_FOR_APPROVAL',0,now(),now())"""),{"run":run_id,"tenant":tenant,"thread":str(run_id)})
    engine.dispose()
    db=DatabaseManager(url);svc=HumanApprovalService(repository=PostgresApprovalRepository(db),signer=ApprovalAuditSigner("phase41-postgres-signing-secret"))
    item=svc.request_review(ApprovalRequestCreate(tenant_id=tenant,run_id=run_id,investigation_id="INV-P41",action_type="production.promote",risk=ApprovalRisk.CRITICAL,risk_score=99,requested_by="planner",evidence_summary=EvidenceSummary(title="critical",summary="live gate"),policy=ReviewPolicy(policy_id="live",required_roles=("release_reviewer",)),idempotency_key="phase41-postgres-gate"))
    ...
    with db.tenant_session(tenant) as session:
        event_id=session.execute(text("SELECT event_id FROM approval_events_phase41 WHERE approval_id=:id ORDER BY sequence LIMIT 1"),{"id":item.approval_id}).scalar_one()
        with pytest.raises(DBAPIError,match="append-only"):
            session.execute(text("UPDATE approval_events_phase41 SET actor_id='mutated' WHERE event_id=:id"),{"id":event_id});session.commit()
        session.rollback()
    # Naked authoritative transition without a matching signed event must fail at commit.
    second_run=uuid4()
    with db.engine.begin() as conn:
        conn.execute(text("""INSERT INTO graph_runs_phase18(run_id,tenant_id,thread_id,graph_name,graph_version,correlation_id,status,last_sequence,created_at,updated_at)
            VALUES(:run,:tenant,:thread,'phase41-live','1','corr2','WAITING_FOR_APPROVAL',0,now(),now())"""),{"run":second_run,"tenant":tenant,"thread":str(second_run)})
    second=svc.request_review(ApprovalRequestCreate(tenant_id=tenant,run_id=second_run,investigation_id="INV-P41-2",action_type="production.promote",risk=ApprovalRisk.CRITICAL,risk_score=99,requested_by="planner",evidence_summary=EvidenceSummary(title="critical-2",summary="signed transition gate"),policy=ReviewPolicy(policy_id="live",required_roles=("release_reviewer",)),idempotency_key="phase41-postgres-gate-2"))
    with pytest.raises(DBAPIError,match="matching signed audit event"):
        with db.tenant_session(tenant) as session:
            session.execute(text("UPDATE approval_requests_phase41 SET status='approved',reviewer_id='bypass',version=version+1,updated_at=now() WHERE approval_id=:id"),{"id":second.approval_id});session.commit()
    db.dispose()
