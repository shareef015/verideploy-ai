"""Phase 25 MCP secure gateway audit persistence."""
...
def upgrade() -> None:
    ...
    op.create_index("ix_mcp_audit_tenant_created", "mcp_tool_audit_phase25", ["tenant_id", "created_at"])
    op.create_index("ix_mcp_audit_tool_decision", "mcp_tool_audit_phase25", ["tenant_id", "tool_name", "decision", "created_at"])
    op.execute(sa.text("ALTER TABLE mcp_tool_audit_phase25 ENABLE ROW LEVEL SECURITY"))
    op.execute(sa.text("ALTER TABLE mcp_tool_audit_phase25 FORCE ROW LEVEL SECURITY"))
    op.execute(sa.text(
        "CREATE POLICY mcp_tool_audit_phase25_tenant_isolation ON mcp_tool_audit_phase25 "
...
def downgrade() -> None:
    op.drop_table("mcp_tool_audit_phase25")
