...
def validate_final_release(root: Path) -> dict[str, Any]:
    ...
    rollback=(root/"scripts/release/phase86_rollback.sh").read_text()
    deploy=(root/"scripts/release/phase86_deploy.sh").read_text()
    ...
    seed=(root/"scripts/release/phase86_seed_demo.sh").read_text()
